%% The data processing stage before calculating the state transition matrix T of the rest task
 

clc;clear;close all;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\rest\REST_ROI_signals\Net_100');        
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\rest\REST_ROI_signals\Net_100\',file_folder(i).name], 'REST_Net100_ROI');    
    
    
    for j = 1:num_brain    
        rest_Net100_ZROI = zscore(REST_Net100_ROI(:,j));     
        rest_all(j,:,i) = rest_Net100_ZROI';                  
    end
...
for i = 1:sub           
    for j = 1:num_brain       
        ...
        rest_all_pca = rest_all(j,:,i);         
        
        std_bold(j,i) = std(rest_all_pca(:));   
...
for i = 1:m                       % 任务态的数量
    for j = 1:sub               % 指刺激的个数  
        for k = 1:num_brain       % num_brain指的是被激活的脑区
...
cell_IDs = cell(1,m);           % 记录激活点脑区编号
spike_times = cell(1,m);        % 记录激活点激活时间
stimuli_lengths = cell(1,m);    % 记录整个任务长度
for i = 1:m
...
save('Data_processed_rest','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain','stimuli_time','std_bold')
