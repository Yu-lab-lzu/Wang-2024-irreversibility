%% The data processing stage before calculating the state transition matrix T of the language task
 

clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\language\network_100_glm');        
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\language\network_100_glm\',file_folder(i).name,'\language_Net_100_ROI.mat'], 'language_Net_100_ROI');
   
    for j = 1:num_brain
        
        language_Net100_ZROI = zscore(language_Net_100_ROI(:,j));      
        language_all(j,:,i) = language_Net100_ZROI';
        
    end
end
language_all(:,177:end,:) = [];
...
task_3 = cell(1,m);         
task_3{1,1} = language_all;

for i = 1:sub     
    for j = 1:num_brain   
        
        language_all_pca = language_all(j,:,i);           
        
        std_bold(j,i) = std(language_all_pca(:));   
...
save('Data_processed_language.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain')
