%% %% The data processing stage before calculating the state transition matrix T of the social task

clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\social\network_100_glm');        
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\social\network_100_glm\',file_folder(i).name,'\SOCIAL_Net_100_ROI.mat'], 'SOCIAL_Net_100_ROI');

    for j = 1:num_brain
        
        social_Net100_ZROI = zscore(SOCIAL_Net_100_ROI(:,j));      
        social_all(j,:,i) = social_Net100_ZROI';
...
task_3 = cell(1,m);         
task_3{1,1} = social_all;

for i = 1:sub    
    for j = 1:num_brain    
        
        social_all_pca = social_all(j,:,i);           
        
        std_bold(j,i) = std(social_all_pca(:));   
...
for i = 1:m        
    ...
    for j = 1:sub      
        ...
        stimuli_lengths{1,i}{1,j} = L.*t;          % 这里stimuli_length是整个事件序列的长度
...
save('Data_processed_social.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain')
