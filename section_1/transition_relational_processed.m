%% The data processing stage before calculating the state transition matrix T of the relational task


clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\relational\network_100_glm');       
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\relational\network_100_glm\',file_folder(i).name,'\RELATIONAL_Net_100_ROI.mat'], 'RELATIONAL_Net_100_ROI');
    
    for j = 1:num_brain
        
        relational_Net100_ZROI = zscore(RELATIONAL_Net_100_ROI(:,j));     
        relational_all(j,:,i) = relational_Net100_ZROI';
        
    end
end
relational_all(:,177:end,:) = [];
...
task_3 = cell(1,m);          
task_3{1,1} = relational_all;

for i = 1:sub     
    for j = 1:num_brain    
        
        relational_all_pca = relational_all(j,:,i);           
        
        std_bold(j,i) = std(relational_all_pca(:));   
...
save('Data_processed_relational.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain')
