%% The data processing stage before calculating the state transition matrix T of the emotion task

clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\emotion\network_100_glm');      
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\emotion\network_100_glm\',file_folder(i).name,'\emotion_Net_100_ROI.mat'], 'emotion_Net_100_ROI');
    
    for j = 1:num_brain
        
        emotion_Net100_ZROI = zscore(emotion_Net_100_ROI(:,j));      
        emotion_all(j,:,i) = emotion_Net100_ZROI';
...
task_3 = cell(1,m);      
task_3{1,1} = emotion_all;

for i = 1:sub     
    for j = 1:num_brain    
        
        emotion_all_pca = emotion_all(j,:,i);           
        
        std_bold(j,i) = std(emotion_all_pca(:));   
...
save('Data_processed_emotion.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain','stimuli_time')
