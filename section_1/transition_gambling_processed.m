%% The data processing stage before calculating the state transition matrix T of the gambling task
 

clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\gambling\network_100_glm');        
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\gambling\network_100_glm\',file_folder(i).name,'\GAMBLING_Net_100_ROI.mat'], 'GAMBLING_Net_100_ROI');
    

    for j = 1:num_brain
        
        gambling_Net100_ZROI = zscore(GAMBLING_Net_100_ROI(:,j));     
        gambling_all(j,:,i) = gambling_Net100_ZROI';
        
    end
end
gambling_all(:,177:end,:) = [];
...
task_3 = cell(1,m);          
task_3{1,1} = gambling_all;

for i = 1:sub    
    for j = 1:num_brain    
        
        gambling_all_pca = gambling_all(j,:,i);           
        
        std_bold(j,i) = std(gambling_all_pca(:));  
...
save('Data_processed_gambling.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain')
