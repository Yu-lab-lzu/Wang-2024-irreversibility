%% The data processing stage before calculating the state transition matrix T of the motor task
 

clear;close all;clc;

file_folder = dir('C:\Users\WANG\Desktop\ENTROPY\motor\network_100_glm');        
...
for i = 1:length(file_folder)
    ...
    file_folder(i).name;
    load(['C:\Users\WANG\Desktop\ENTROPY\motor\network_100_glm\',file_folder(i).name,'\motor_Net_100_ROI.mat'], 'motor_Net_100_ROI');
    
    for j = 1:num_brain
        
        motor_Net100_ZROI = zscore(motor_Net_100_ROI(:,j));      
        motor_all(j,:,i) = motor_Net100_ZROI';
...
task_3 = cell(1,m);        
task_3{1,1} = motor_all;

for i = 1:sub     
    for j = 1:num_brain    
        
        motor_all_pca = motor_all(j,:,i);           
        
        std_bold(j,i) = std(motor_all_pca(:));   
...
save('Data_processed_motor.mat','cell_IDs','spike_times','stimuli_lengths','task_3','num_brain')
